 ...
 */
package Lumileds.cockpits.setup;
...
import Lumileds.cockpits.constants.LumiledsCockpitsConstants;
 ...
 */
@SystemSetup(extension = LumiledsCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(LumiledsCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(LumiledsCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						LumiledsCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/Lumiledscockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
